...
export class ConsumerService {
    ...
    try {
        ...
        this.logger.log(`Created consumer group "${GROUP_NAME}" on ${REDIS_KEYS.STREAM_EVENTS}`);
    } catch (err: unknown) {
    ...
    if (!event?.apiKeyId || !event.organizationId) {
        this.logger.warn(
            `Bad payload for ${messageId} (missing apiKeyId or organizationId), ACKing to drop`,
        );
        await this.redis.xack(REDIS_KEYS.STREAM_EVENTS, GROUP_NAME, messageId);
        return;
    }
    ...
    while (!this.stopRequested) {
        ...
        if (!event.apiKeyId || !event.organizationId) {
            this.logger.warn('Skipping fallback event missing apiKeyId or organizationId');
            continue;
        }
